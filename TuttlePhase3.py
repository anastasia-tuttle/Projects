while True:
    print("\nWELCOME")
    print("1. Sudoku")
    print("2. High roll vs Low roll")
    print("3. Exit")
    choice1=int(input("Please pick a game."))
    ...
    if choice1==1:
                    ...
                    for j in range(0, len(grid_cols)):
                        if user_input[1] == grid_cols[j]:
                            if puzzle[i][j] != 0:  # Call the eraseEntry function and ask
                                eraseEntry(
                                    puzzle)  # Ask the user if they want to erase the entry where there isn't a zero
                            puzzle[i][j] = int(get_input)
                            make_move = turtle.write(get_input, move=False, align='left',
        ...
        def populatePuzzle(puzzle):
            ...
            col_increment_size = [-60, -30, 10,
                                    50]  # Starts at A1 (-60,50), A2 (-30,50), A3 (10,50) .... D4 (50,-60)
            row_increment_size = [50, 10, -30, -60]
        ...
        def col_check(puzzle):
            ...
            col_1 = [s[0] for s in
                        puzzle]  # Turns each 'row' of the puzzle into a column. Kind of how you invert a matrice
            col_2 = [s[1] for s in
                        puzzle]  # so for example. col_1 = [A1, B1, C1, D1], col_2 = [A2, B2, C2, D2] and so on.
            col_3 = [s[2] for s in
                        puzzle]  # After the transformation is done, it applies the checker. The checker looks
            col_4 = [s[3] for s in
                        puzzle]  # at the length of each column. If every number is unique len(set(col_1)) = 4 and
        ...
        def box_check(puzzle):
            ...
            box_1 = [s[0:2] for s in
                        puzzle[
                        0:2]]  # Transforming each row of the puzzle so that all elements are rearranged as boxes
            box_2 = [s[2:] for s in
                     puzzle[0:2]]  # so for example, box_1 = [A1,A2,B1,B2], box_2 = [A3,A4,B3,B4] and so on
        ...
        while True:
            ...
            make_box()
            turtle.goto(-95,
                        45)  # Start by going to the left top of the grid (Labeling starts with 'A', 'B'... etc)
            labelGrid()
    ...
    if choice1==2:
        ...
        while True:
            ...
            print("1) High Roll (8-12)   (2x BET)")
            print("2) Low Roll  (1-6)    (2x BET)")
            print("3) 7                  (3x BET)")
            ...
            if choice == 'x' or choice == 'X':
                ...
                if roll > 7 and choice == '1':
                    ...
                    cash = cash - bet
                    if cash == 0:
                        print("You ran out of money!! See you next time!!")
                        break
            else:
                print("Enter an invalid choice!")
                continue
